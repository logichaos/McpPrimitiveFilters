...
This sample demonstrates how to create an MCP server
using the McpServerSDK that requires OAuth 2.0 authentication to access its tools and resources. The server provides weather-related tools protected by JWT bearer token authentication.

## Overview

The McpServerSDK sample shows how to:
...
You can test the server using the McpClient sample:

```bash
cd samples\McpClient
dotnet run
```
