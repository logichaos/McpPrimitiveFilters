...
public sealed class Program
{
    ...
    // Port 5000 is used by tests and port 7071 is used by the McpServerSDK sample
    ...
    public async Task RunServerAsync(string[]? args = null, CancellationToken cancellationToken = default)
    {
            ...
            [FromQuery] string? resource) =>
        {
            ...
            if (ExpectResource ? (string.IsNullOrEmpty(resource) || !ValidResources.Any( vr => new Uri(vr).Equals(new Uri(resource)))) : !string.IsNullOrEmpty(resource))
            {
