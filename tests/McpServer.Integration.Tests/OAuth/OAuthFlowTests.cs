...
public class OAuthFlowTests : OAuthTestBase
{
  private string ClientMetadataDocumentUrl => $"{OAuthServerUrl}/client-metadata/cimd-client.json";
  ...
  public async Task CanAuthenticate_WithTokenRefresh()
  {
    ...
    await Assert.That(TestOAuthServer.State.HasRefreshedToken).IsTrue();
  }
